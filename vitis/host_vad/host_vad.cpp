...
#define OUTPUT_FILE "output_vad.csv"
#define CONFIG_FILE "config_vad.txt"
#define INPUT_ORIGIN_FILE "data_non_damage_scaled_processed.csv"
#define HILBERT_ORIGIN_FILE "clean_output.txt"
...
bool load_signals_from_file(const std::string& filename, float* signals);
bool load_signals_from_file_hilbert(const std::string& filename, float* signals);
void calcularDistanciasEnHilo(float* distancias_E_host, float* distancias_R_host);
void loadOriginalDataThread(float* ptr_input_orig);
void loadHilbertDataThread(float* ptr_hilbert);
...
int main(int argc, char* argv[]) {
    ...
    std::vector<std::complex<float>> accumulated_damage(INDEX_DAMAGE_SIZE, {0.0f, 0.0f});
    std::vector<float> final_damage_result(INDEX_DAMAGE_SIZE, 0.0); 
    std::vector<float> final_damage_result2(INDEX_DAMAGE_SIZE, 0.0); 
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_vad = cl::Kernel(program, "krnl_vad", &err));
            valid_device = true;
    ...
    ptr_input_orig = new float[TOTAL_SIZE];  
    ptr_hilbert = new float[TOTAL_SIZE];  
    ...
    OCL_CHECK(err, cl::Buffer buffer_input_orig(context, CL_MEM_READ_ONLY, size_in_bytes_input, NULL, &err));
    OCL_CHECK(err, cl::Buffer buffer_hilbert(context, CL_MEM_READ_ONLY, size_in_bytes_input, NULL, &err));
    ...
    int narg = 0;
    OCL_CHECK(err, err = krnl_vad.setArg(narg++, buffer_input));
    OCL_CHECK(err, err = krnl_vad.setArg(narg++, buffer_input_orig));
    OCL_CHECK(err, err = krnl_vad.setArg(narg++, buffer_hilbert));
    OCL_CHECK(err, err = krnl_vad.setArg(narg++, buffer_dist_E));
    OCL_CHECK(err, err = krnl_vad.setArg(narg++, buffer_dist_R));
    OCL_CHECK(err, err = krnl_vad.setArg(narg++, buffer_result));
    OCL_CHECK(err, err = krnl_vad.setArg(narg++, DATA_SIZE));
    OCL_CHECK(err, err = krnl_vad.setArg(narg++, INDEX_DAMAGE_SIZE));
    OCL_CHECK(err, err = krnl_vad.setArg(narg++, NUM_SIGNALS_SIZE));


    OCL_CHECK(err,
              ptr_input = (float*)q.enqueueMapBuffer(buffer_input, CL_TRUE, CL_MAP_WRITE, 0, size_in_bytes_input, NULL, NULL, &err));
    OCL_CHECK(err,
              ptr_input_orig = (float*)q.enqueueMapBuffer(buffer_input_orig, CL_TRUE, CL_MAP_WRITE, 0, size_in_bytes_input, NULL, NULL, &err));
    OCL_CHECK(err,
              ptr_hilbert = (float*)q.enqueueMapBuffer(buffer_hilbert, CL_TRUE, CL_MAP_WRITE, 0, size_in_bytes_input, NULL, NULL, &err));
    OCL_CHECK(err,
    ...
    std::thread original_data_thread(loadOriginalDataThread, ptr_input_orig);
    std::thread hilbert_data_thread(loadHilbertDataThread, ptr_hilbert);

    producer_thread.join();
    consumer_thread.join();
    distancias_thread.join();
    original_data_thread.join();
    hilbert_data_thread.join();
    
    // Data will be migrated to kernel space*/   
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_input,buffer_input_orig,buffer_hilbert,buffer_dist_E,buffer_dist_R}, 0 /* 0 means from host*/));

    for (int i = 0; i < N_iter; i++) {
        auto start_time = std::chrono::high_resolution_clock::now();
        cl::Event event;
    
        OCL_CHECK(err, q.enqueueTask(krnl_vad, nullptr, &event));
        event.wait();
    ...
    OCL_CHECK(err, err = q.enqueueUnmapMemObject(buffer_input_orig, ptr_input_orig));
    OCL_CHECK(err, err = q.enqueueUnmapMemObject(buffer_hilbert, ptr_hilbert));
...
// **Función para cargar señales desde un archivo CSV usando punteros**
bool load_signals_from_file(const std::string& filename, float* signals) {
    ...
    std::string line;
    int count = 0;
    while (count < NUM_SIGNALS && std::getline(infile, line)) {
        std::stringstream ss(line);
        float value;
        int index = count * FFT_LENGTH;

        for (int i = 0; i < FFT_LENGTH; i++) {
            ss >> value;
            signals[index + i] = value;
...
bool load_signals_from_file_hilbert(const std::string& filename, float* signals) {
    ...
    while (count < NUM_SIGNALS && std::getline(infile, line)) {
        ...
        while (ss >> value && i < FFT_LENGTH) {
            signals[count * FFT_LENGTH + i] = value;
            if (ss.peek() == ',') ss.ignore();
            i++;
        }

        if (i != FFT_LENGTH) {
            std::cerr << "Error: Línea " << count << " tiene " << i << " muestras (esperadas: 4096)\n";
            return false;
        }
...
void get_emisor_receptor(int index_signal, int& E, int& R) {
    ...
    for (int e = 1; e <= NUM_SENSORES; e++) {
        for (int r = 1; r <= NUM_SENSORES; r++) {
            if (e != r) {  // Evitar que E y R sean el mismo sensor
...
void loadOriginalDataThread(float* ptr_input_orig) {
    if (!load_signals_from_file(INPUT_ORIGIN_FILE, ptr_input_orig)) {
        std::cerr << "Error al cargar las señales de entrada original\n";
        return;
    }
}

void loadHilbertDataThread(float* ptr_hilbert) {
    if (!load_signals_from_file_hilbert(HILBERT_ORIGIN_FILE, ptr_hilbert)) {
        std::cerr << "Error al cargar las señales Hilbert\n";
        return;
    }
}
